...
import { FileText, CheckCircle2, XCircle, Clock, Code2, ExternalLink } from 'lucide-react'
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      {submissions.length === 0 ? (
        ...
        >
          ...
          <p className="text-gray-500 text-sm">Start solving problems to see your submissions here</p>
          <Link
            to="/"
        ...
        <div className="space-y-4">
          {submissions.map((submission, index) => {
            const VerdictIcon = verdictIcons[submission.verdict as keyof typeof verdictIcons] || Clock
            const verdictClass = verdictColors[submission.verdict as keyof typeof verdictColors] || verdictColors.PENDING
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-4 mb-4">
                      ...
                      >
                        ...
                        <span className="font-semibold">Problem {submission.problemId.slice(0, 8)}</span>
                        <ExternalLink className="w-4 h-4" />
                      </Link>
                      <span className={`px-3 py-1 rounded-full text-xs font-semibold border flex items-center gap-1 ${verdictClass}`}>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                      ...
                      <div>
                        ...
                        <p className="text-white font-medium">{submission.score}</p>
                      </div>
                      {submission.executionTimeMs && (
                        <div>
                          <span className="text-gray-400">Time:</span>
                          <p className="text-white font-medium">{submission.executionTimeMs}ms</p>
                        </div>
                      )}
                      <div>
                    ...
                    {submission.code && (
                      <details className="mt-4">
                        ...
                        <pre className="mt-2 p-4 bg-black/30 rounded-lg overflow-x-auto text-xs text-gray-300">
                          {submission.code.slice(0, 200)}{submission.code.length > 200 ? '...' : ''}
                        </pre>
                      </details>
                    )}
                  </div>
